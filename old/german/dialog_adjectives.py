...
df = pd.read_csv("data_adjectives.csv")
print(df.head())
...
def get_a_word() -> (dict, list):
    ...
    btn1.setText(lst_words[0].replace(', ', "\n"))
    btn2.setText(lst_words[1].replace(', ', "\n"))
    btn3.setText(lst_words[2].replace(', ', "\n"))
    btn4.setText(lst_words[3].replace(', ', "\n"))
    text_to_speak = word["adjective"]
    print(word)
    print(lst_words)
...
def create_string_word(color):
    ...
    label_word.setText(f"{active_word['adjective']}")
    #label_tranlation.setText(f"({active_word['translation']})")
...
def create_string_result(color):
    global active_word
    global text_to_speak
    text = f"{active_word['adjective']}, {active_word['comparativo']}, {active_word['superlativo']}"        
...
window.setWindowTitle("Adjektive") 
