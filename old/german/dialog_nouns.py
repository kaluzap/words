...
def create_string_result(color):
    ...
    global text_to_speak
    text = ""
    if active_word['word'] == 'ohne':
        text += '[ohne s.], '
    else:
        if active_word['gender'] == 'm':
            text += 'der '
        elif active_word['gender'] == 'f':
            text += 'die '
        else:
            text += 'das '
        text += active_word['word'] + ", "
        
    if active_word['plural'] == 'ohne':
        text += "[ohne pl.]"
    else:
        text += f"die {active_word['plural']}"
